...
<!--设置菜单图标样式-->
{{assign var=themIcon value=["fa-th-large",  "fa-file-o",  "fa-search-minus", "fa-dribbble", "fa-windows", "fa-apple",  "fa-arrow-circle-o-up",  "fa-list-alt", "fa-gavel",  "fa-maxcdn",  "fa-flag-o",  "fa-chevron-circle-up",  "fa-xing",  "fa-tumblr"]}}
<header class="bg-dark dk header navbar navbar-fixed-top-xs">
	<div class="navbar-header aside-md" >
		<a class="btn btn-link visible-xs" data-toggle="class:nav-off-screen,open" data-target="#nav,html"><i class="fa fa-bars"></i></a>
		<a  class="navbar-brand" data-toggle="fullscreen"><img src="{{$STATIC_HOST}}/css/admin/notebook/notebook_files/logo.png" class="m-r-sm">Mi-Gang</a>
		<a class="btn btn-link visible-xs" data-toggle="dropdown"data-target=".nav-user"> <i class="fa fa-cog"></i></a>
	</div>
	<ul class="nav navbar-nav hidden-xs">
		{{foreach from=$userBigMenuList item=bigMenu}}
			<li>
				<a class="dker" href="javascript:void(0);"  onclick="getSubMenu('{{$bigMenu['name']}}', {{$bigMenu['id']}});"> <i class="fa {{$themIcon[$bigMenu['id']%14]}}"></i> <span class="font-bold">{{$bigMenu['name']}}</span></a>
			</li>
		{{/foreach}}
	</ul>
	<ul class="nav navbar-nav navbar-right m-n hidden-xs nav-user" style="cursor:pointer;">
				...
				<section class="panel bg-white">
					...
					<div class="list-group list-group-alt animated fadeInRight">
						...
						<a class="media list-group-item">
							<span class="pull-left thumb-sm"><img src="{{$STATIC_HOST}}/css/admin/notebook/notebook_files/avatar.jpg" alt="John said" class="img-circle"></span> 
		...
		<li class="dropdown">
			<a class="dropdown-toggle" data-toggle="dropdown"> 
				<span class="thumb-sm avatar pull-left"> <img src="{{$STATIC_HOST}}/css/admin/notebook/notebook_files/avatar.jpg"></span> {{$userInfo['username']}} <b class="caret"></b>
			</a>
			<ul class="dropdown-menu animated fadeInRight">
					...
					<li><a>帮助</a></li>
					<li class="divider"></li>
					<li><a href="{{$ADMIN_HOST}}/setup/user/logout">退出</a></li>
				</ul>
		</li>
	</ul>
	<script>
		...
		function updPwd(){
			...
			layer.open({
			  ...
			  type: 2,
			  area: ['720px', '420px'],
			  content: ['{{$ADMIN_HOST}}/setup/user/edit/pwd', 'no'],
			});
		}
	...
	$.ajax({
              type: 'post',
              url: '{{$ADMIN_HOST}}/setup/get/sub/menu',
              dataType: 'json',
              timeout: 60000, 
              data:{
		pid: pid
   		...
   		if(menuData.errno == 0 && menuData.subMenuList.length > 0){
   			...
   			for(var i = 0;i<subMenuList.length;i++){
   				...
   				for(var j = 0; j<subMenuList[i].child.length; j++){
   					html += 		'<li><a href="javascript:void(0)"  onclick="Common.loadPage(\''+menuName+'\', \''+subMenuList[i].name+'\',\''+subMenuList[i].child[j].name+'\',\'{{$ADMIN_HOST}}'+subMenuList[i].child[j].url+'\', true)"> <i class="fa fa-angle-right"></i> <span>'+subMenuList[i].child[j].name+'</span></a></li>';
   				}
   				html += 	'</ul>';
   				html += '</li>';
   			}
