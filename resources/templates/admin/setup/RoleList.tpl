...
<script style="text/javascript">
	...
	$('#addAccount').click(function(){
		...
		layer.open({
			  ...
			  type: 2,
			  area: ['720px', '357px'],
			  content: ['{{$ADMIN_HOST}}/setup/role/add', 'no'],
			}); 
	});
	...
	function setRolePpriv(roleName, roleId){
		...
		layer.open({
			  ...
			  type: 2,
			  area: ['720px', '450px'],
			  content: ['{{$ADMIN_HOST}}/setup/role/permit?roleId='+roleId],
		}); 
	}
	...
	function roleUser(roleName, roleId){
		...
		layer.open({
			  ...
			  type: 2,
			  area: ['720px', '485px'],
			  content: ['{{$ADMIN_HOST}}/setup/role/user?roleId='+roleId],
		}); 
	}
	...
	function editRole(roleId){
		...
		layer.open({
		  ...
		  type: 2,
		  area: ['720px', '357px'],
		  content: ['{{$ADMIN_HOST}}/setup/role/edit?roleId='+roleId, 'no'],
		}); 
	}
	//页面追加数据
	...
	function getListData(pageNum){
		...
		var pageSize = 9;
		 ...
		 //加载数据
		  $.ajax({
	              type: 'post',
	              url: '{{$ADMIN_HOST}}/setup/get/role/list',
	              dataType: 'json',
	              data:{
	            	  page: page, 
	            	  pageSize: pageSize
