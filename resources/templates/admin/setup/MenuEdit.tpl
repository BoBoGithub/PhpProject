...
<section class="panel panel-default">
			<div class="panel-body">
					...
					<div class="col-sm-9">
						<div class="dropdown">  
								<button class="btn btn-default" data-toggle="dropdown" id="dropdownBtn">  
									<span id="menText">{{$parentMenu['name']}}</span>  
									<span class="caret"></span>  
								</button>  
								<input type="hidden" value="{{$parentMenu['id']}}" id="parentId" />
				...
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" class="form-control" placeholder="请输入菜单名称" name="menuName" id="menuName" value="{{$menuData['name']}}">
					</div>
				</div>
				<div class="line line-dashed line-lg pull-in"></div>
				<div class="form-group">
					...
					<div class="col-sm-9">
						<input type="text" class="form-control" placeholder="请输入请求地址" name="requestUrl" id="requestUrl" value="{{$menuData['url']}}">
					</div>
				</div>
				<div class="line line-dashed line-lg pull-in"></div>
 				<div class="form-group">
					...
					<label class="radio-inline">
						<input type="radio" name="menuStatus" value="0" {{if $menuData['status'] == 0}}checked{{/if}}> 是
					</label>
					<label class="radio-inline">
						<input type="radio" name="menuStatus"  value="1" {{if $menuData['status'] == 1}}checked{{/if}}> 否
					</label>
				</div>
			</div>
		...
		<script>
			...
			$.ajax({
			      type: 'post',
		              url: '{{$ADMIN_HOST}}/setup/edit/menu',
		              dataType: 'json',
		              timeout: 60000, 
		              data:{
		            	  menuId: {{$menuData['id']}},
