...
<section class="panel panel-default">
			<div class="panel-body">
				<div class="form-group">
					...
					<div class="col-sm-9">
						<div class="dropdown">  
							{{if !empty($currRoleData)}}
								<button class="btn btn-default" data-toggle="dropdown" id="dropdownBtn">  
									<span id="roleText">{{$currRoleData['rolename']}}</span>  
									<span class="caret"></span>  
								</button>  
								<input type="hidden" value="{{$currRoleData['roleid']}}" id="roleId" />
							{{else}}
								<button class="btn btn-default" data-toggle="dropdown" id="dropdownBtn">  
							...
							{{/if}}
								<ul class="dropdown-menu" style="max-height: 285px; overflow-y: auto;cursor:pointer;" id="roleSelect">  
									<li data-original-index="0" {{if empty($currRoleData)}}class="selected active"{{/if}}>
										<a>选择角色</a>
									</li>
									{{foreach from=$roleListData item=roleData}}
	...
	<script>
		...
		$.ajax({
	              type: 'post',
	              url: '{{$ADMIN_HOST}}/setup/user/post/edit',
	              dataType: 'json',
	              cache:false,
	              timeout: 60000, 
	              data:{
